...
6. Cloudflare Pages に本番 secrets / vars を設定する
...
`SESSION_SECRET` は次で生成できます。

```bash
npm run generate:session-secret
```

`gh` 認証済みなら GitHub Secrets は次で入れられます。
...
- D1 本番 DB は作成済み
- D1 スキーマ投入済み
- Supabase から吸い出した CSV の本番投入済み
- 画像 URL は旧 Supabase / X(Twitter) CDN の混在状態

画像については、現時点では「取得できたものだけ R2 に寄せる」ための補助スクリプトを入れていますが、未取得画像が残っていてもサイト自体の移行は進められます。
先に本体の配信先を Cloudflare Pages に切り替えて、その後に画像だけ段階的に整理する進め方で問題ありません。
